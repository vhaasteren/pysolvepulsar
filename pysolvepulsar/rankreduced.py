...
#TODO: This is a quick hack to get the inverse. Write it properly
def get_rr_CiA(N, Fmat, psd, A):
    """C = N + F*psd*F^T  Calculate C^{-1}A"""
    n, m, l = Fmat.shape[0], Fmat.shape[1], A.shape[1]
    R = np.zeros((n, l))
    
    for ii in range(l):
        x = get_rr_Lix(N, Fmat, psd, A[:,ii])
        R[:,ii] = get_rr_Uix(N, Fmat, psd, x)
