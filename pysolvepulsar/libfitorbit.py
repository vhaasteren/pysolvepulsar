...
from btmodel import BTmodel, Cache
...
def BT_period(t, P0, P1, PEPOCH, PB, ECC, A1, T0, OM, RA_RAD, DEC_RAD, \
    ...
    btm = BTmodel(t, P0, P1, PB, PBDOT, ECC, OM, A1, T0, EDOT, OMDOT, PEPOCH)
    return btm.Pobs()

@Cache.use_cache
def BT_period_designmatrix(t, P0, P1, PEPOCH, PB, ECC, A1, T0, OM, RA_RAD, DEC_RAD, \
        EDOT=0.0, PBDOT=0.0, OMDOT=0.0):
    btm = BTmodel(t, P0, P1, PB, PBDOT, ECC, OM, A1, T0, EDOT, OMDOT, PEPOCH)
    ...
    M[:,2] = 0.0
    M[:,3] = btm.d_Pobs_d_PB()
    M[:,4] = btm.d_Pobs_d_ECC()
    M[:,5] = btm.d_Pobs_d_A1()
    M[:,6] = btm.d_Pobs_d_T0()
    M[:,7] = btm.d_Pobs_d_OM()
    M[:,8] = 0.0
    M[:,9] = 0.0
    M[:,10] = btm.d_Pobs_d_EDOT()
    M[:,11] = btm.d_Pobs_d_PBDOT()
    M[:,12] = btm.d_Pobs_d_OMDOT()

    return M


def BT_period_old(t, P0, P1, PEPOCH, PB, ECC, A1, T0, OM, RA_RAD, DEC_RAD, \
        EDOT=0.0, PBDOT=0.0, OMDOT=0.0):
    """
...
class orbitpulsar(object):
    ...
    def initParameters(self):
        ...
        self.bmparams['BT'] = ['RA', 'DEC', 'P0', 'P1', 'PEPOCH', 'PB', 'ECC',
                'A1', 'T0', 'OM', 'OMDOT', 'EDOT']
    ...
    def readParFile(self, parfilename):
        ...
        if parfilename is not None and os.path.isfile(parfilename):
            ...
            if hasattr(self.parf, 'OMDOT'):
                self['OMDOT'].val = np.float128(self.parf.OMDOT)
            else:
                self['OMDOT'].val = np.float128(0.0)

            if hasattr(self.parf, 'EDOT'):
                self['EDOT'].val = np.float128(self.parf.EDOT)
            else:
                self['EDOT'].val = np.float128(0.0)
        else:
    ...
    def orbitDesignMatrix(self, mjds=None, pardict=None, parameters=None, which='set',
            parlist=None, model=None):
        if mjds is None:
            mj = self.mjds
        else:
        ...
        if model in ['BT']:
            M = BT_period_designmatrix(mj, *bmarr)

            # Only usable columns are
            inds = np.array([0, 1, 3, 4, 5, 6, 7, 12])
            return M[:,inds]
        elif model == 'RED':
            raise NotImplemented("Only BT works for now")
        elif model  == 'DD':
    ...
    def searchExtraPeriodicity(self, pbmin, pbmax, pbdelta):
        """
        Given the current solution, see if there is an extra periodicity in the
        data
        """
        # TODO: Remove this bit again
        pb2s = np.linspace(pbmin, pbmax, int((pbmax-pbmin)/pbdelta))
        chi2 = np.zeros_like(pb2s)

        res = self.orbitResiduals()
        M1 = self.orbitDesignMatrix()
        M2 = np.zeros((len(res), 2))        # Periodicity of sine & cosine

        for ii, pb2 in enumerate(pb2s):
            M2[:,0] = np.sin(2*np.pi*self.mjds/pb2)
            M2[:,1] = np.cos(2*np.pi*self.mjds/pb2)
            M = np.append(M1, M2, axis=1)
            Nvec = self.periodserrs**2 if self.periodserrs is not None \
                    else np.ones_like(res)

            MNM = np.dot(M.T / Nvec, M)
            MNMi = sl.inv(MNM)
            newpar = np.dot(MNMi, np.dot(M.T, res / Nvec))

            newres = res - np.dot(M, newpar)

            chi2[ii] = np.sum(newres**2/Nvec)
